...
const AdminUserForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await axios.get(' http://20.84.40.134:8080/api/admin-users/roles/');
  ...
  const onSubmit = async (data: AdminUserFormValues) => {
    try {
      const response = await axios.post(' http://20.84.40.134:8080/api/admin-user/add/', data);
