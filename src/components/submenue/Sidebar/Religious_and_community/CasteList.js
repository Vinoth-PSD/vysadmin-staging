...
const CasteList = () => {
    ...
    const fetchCastes = async () => {
        const response = await axios.get('http://20.246.74.138:5173/api/castes/');
        setCastes(response.data);
    };

    const addCaste = async () => {
        if (newCaste.name.trim() === '') return; 
        await axios.post('http://20.246.74.138:5173/api/castes/', newCaste);
        setNewCaste({ name: '' });
        setShowPopup(false);
    ...
    const deleteCaste = async (id) => {
        await axios.delete(`http://20.246.74.138:5173/api/castes/${id}/`);
        fetchCastes();
    };

    const editCaste = async () => {
        if (editingCaste.name.trim() === '') return;
        await axios.put(`http://20.246.74.138:5173/api/castes/${editingCaste.id}/`, editingCaste);
