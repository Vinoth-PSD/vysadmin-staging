...
const ReligionList = () => {
    ...
    const fetchReligions = async () => {
        const response = await axios.get('http://20.246.74.138:5173/api/religions/');
        setReligions(response.data);
    };

    const handleAddReligion = async () => {
        if (newReligion.trim() === '') return; // Prevent empty submission
        await axios.post('http://20.246.74.138:5173/api/religions/', { name: newReligion });
        setNewReligion('');
        setShowPopup(false);
    ...
    const confirmDeleteReligion = async () => {
        await axios.delete(`http://20.246.74.138:5173/api/religions/${religionToDelete}/`);
    ...
    const handleUpdateReligion = async () => {
        ...
        await axios.put(`http://20.246.74.138:5173/api/religions/${editReligionId}/`, { name: editedReligionName });
